...
watch(props, async (value) => {
  ...
  value.state.active ? await new Promise((resolve) => setTimeout(resolve, 500)) : null
...
<template>
  <transition name="fadeIn">
    ...
    <div class="w-full h-full relative flex items-center justify-center">
      <div @click="close" class="overlay z-0 position absolute w-full h-full top-0 left-0"></div>
      <transition name="zoom">
        <div v-show="isWhiteActive" class="bg-white px-32 py-10 opacity-90 flex flex-col text-center">
          <div class="mb-10">
          ...
          <div class="mt-10 my-5">
            <button class="button" @click="props.state.toHome">
              casa
            </button>
            <button class="button ml-[83px]" @click="props.state.toHome">
              aluxion
            </button>
          </div>
        </div>
